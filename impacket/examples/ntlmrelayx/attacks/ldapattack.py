...
class LDAPAttack(ProtocolAttack):
    ...
    # 传入usersid，传出域当中该createerID为该用户的机器账户信息
        entries = self.client.search(domainDumper.root, '(mS-DS-CreatorSID=%s)' % escape_filter_chars(usersid),
        ...
        for i in range(0,len(self.client.entries)):
            try:
                dn = self.client.entries[i].entry_dn
                sid = self.client.entries[i]['objectSid']
